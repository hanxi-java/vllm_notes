...
1. [MemCache 配置说明](#memcache-配置说明)
   - [MetaService（元数据服务）配置](#metaservice元数据服务配置)
   - [LocalService（本地服务）配置](#localservice本地服务配置)
2. [多级缓存：淘汰与回温（Rewarm）机制详解](#多级缓存淘汰与回温rewarm机制详解)
   - [问题 1：按 LRU 淘汰，具体的淘汰计算数量和规则是怎么样的？](#问题-1按-lru-淘汰具体的淘汰计算数量和规则是怎么样的)
     - [数据结构：每层一条独立的 LRU 链表](#数据结构每层一条独立的-lru-链表)
     - [淘汰触发与数量计算](#淘汰触发与数量计算)
   - [问题 2：Get 路径同步回温和 ExistKey 路径异步回温的触发条件是什么？过程如何？](#问题-2get-路径同步回温和-existkey-路径异步回温的触发条件是什么过程如何)
     - [Get 路径详细流程（`FillObjMetaWithRewarm`）](#get-路径详细流程fillobjmetawithrewarm)
     - [ExistKey 路径详细流程](#existkey-路径详细流程)
   - [问题 3：可读 blob 是什么概念？blob 是什么概念？](#问题-3可读-blob-是什么概念blob-是什么概念)
   - [问题 4：Get 时遍历 blob 的详细过程](#问题-4get-时遍历-blob-的详细过程)
   - [问题 5：回温执行（RewarmBlob）的详细过程](#问题-5回温执行rewarmblob的详细过程)
3. [Q&A：MemcacheBackend 的创建、数量与 key 语义](#qamemcachebackend-的创建数量与-key-语义)
   - [1. "每个 NPU rank 创建一个 store 实例并初始化"的代码依据](#1-每个-npu-rank-创建一个-store-实例并初始化的代码依据)
   - [2. MemcacheBackend 对象会创建几次？（DP=2, PP=2, TP=2）](#2-memcachebackend-对象会创建几次dp2-pp2-tp2)
   - [3. Key 语义详解：`model@pcp@dcp@tp_rank@pp_rank@group@cache_role@cache_family@chunk_hash`](#3-key-语义详解modelpcpdcptp_rankpp_rankgroupcache_rolecache_familychunk_hash)
4. [Q&A：MemcacheBackend 的作用与请求处理全链路](#qamemcachebackend-的作用与请求处理全链路)
   - [一、MemcacheBackend 在 KV cache 传输中的作用](#一memcachebackend-在-kv-cache-传输中的作用)
   - [二、从请求进来到与 memcache 发生关系的完整链路](#二从请求进来到与-memcache-发生关系的完整链路)
   - [关键点提炼](#关键点提炼)
5. [Python 客户端接口：DistributedObjectStore、layers、KeyInfo、GVA 与监控](#python-客户端接口distributedobjectstorelayerskeyinfogva-与监控)
   - [问题 1：DistributedObjectStore 类主要功能是怎么样的？](#问题-1distributedobjectstore-类主要功能是怎么样的)
   - [问题 2：DistributedObjectStore 类的 put_from_layers，这里的 layers 是什么概念？](#问题-2distributedobjectstore-类的-put_from_layers这里的-layers-是什么概念)
   - [问题 3：DistributedObjectStore 类的 get_key_info 返回的 KeyInfo 对象，每个字段的含义](#问题-3distributedobjectstore-类的-get_key_info-返回的-keyinfo-对象每个字段的含义)
   - [问题 4：doc/memcache_python_api.md 文档中"GVA 相关接口与常量"中的 GVA 是什么意思？](#问题-4docmemcache_python_apimd-文档中gva-相关接口与常量中的-gva-是什么意思)
   - [问题 5：memcache 有暴露监控能力或者接口吗？比如查询各级存储剩余容量的功能](#问题-5memcache-有暴露监控能力或者接口吗比如查询各级存储剩余容量的功能)
     - [容量查询](#容量查询)
     - [指标监控（Prometheus 格式）](#指标监控prometheus-格式)
     - [运维/调试接口](#运维调试接口)
6. [Q&A：MemcacheBackend 加载配置、head_or_tp_rank 与 chunk_hash 详解](#qamemcachebackend-加载配置head_or_tp_rank-与-chunk_hash-详解)
   - [1. `MemcacheBackend` 创建代码的原文 & 启动参数怎么配](#1-memcachebackend-创建代码的原文--启动参数怎么配)
     - [原文代码](#原文代码)
     - [启动参数配置](#启动参数配置)
   - [2. `head_or_tp_rank` / `put_step` 详细解释](#2-head_or_tp_rank--put_step-详细解释)
     - [涉及的全部代码](#涉及的全部代码)
     - [设计动机：区分"数据不同"和"数据重复"两种 TP 场景](#设计动机区分数据不同和数据重复两种-tp-场景)
   - [3. `chunk_hash`（`request.block_hashes` 前缀链式哈希）详细解释](#3-chunk_hashrequestblock_hashes-前缀链式哈希详细解释)
     - [vLLM 侧：哈希怎么算出来的](#vllm-侧哈希怎么算出来的)
     - [数值例子](#数值例子)
     - [vllm-ascend 侧：粒度对齐](#vllm-ascend-侧粒度对齐)
7. [KV cache 张量布局 与 按介质精确删除 key](#kv-cache-张量布局-与-按介质精确删除-key)
   - [问题 1：KV cache 张量形状 (layer_number=10, block_number=10, block_size=1024) 请再好好解释一下？](#问题-1kv-cache-张量形状-layer_number10-block_number10-block_size1024-请再好好解释一下)
     - [三个维度分别是什么](#三个维度分别是什么)
     - [关键问题：一个 block 的完整 KV 在内存里是**不连续**的](#关键问题一个-block-的完整-kv-在内存里是不连续的)
     - [put_from_layers 解决的就是这个"分散"问题](#put_from_layers-解决的就是这个分散问题)
   - [问题 2：假如想精确地删掉 memcache 上存储的某个介质上的某个 key，能做到吗？](#问题-2假如想精确地删掉-memcache-上存储的某个介质上的某个-key能做到吗)
     - [对外接口：只能删整个 key](#对外接口只能删整个-key)
     - [内部机制：按介质过滤删除是存在的](#内部机制按介质过滤删除是存在的)
     - [实际可行的替代办法](#实际可行的替代办法)
8. [深入：容器结构、Insert 触发、batch_alloc vs batch_put_from、Get 回温性能、高低层含义、MmcMemObjMeta](#深入容器结构insert-触发batch_alloc-vs-batch_put_fromget-回温性能高低层含义mmcmemobjmeta)
   - [问题 1：MmcMetaContainerLRU 对象的结构是什么样的？](#问题-1mmcmetacontainerlru-对象的结构是什么样的)
   - [问题 2：MmcMetaContainer 的子类除了 MmcMetaContainerLRU，还有其他的吗？](#问题-2mmcmetacontainer-的子类除了-mmcmetacontainerlru还有其他的吗)
   - [问题 3：MmcMetaContainerLRU 的 Insert 方法是由谁触发的？blobType 是什么概念？](#问题-3mmcmetacontainerlru-的-insert-方法是由谁触发的blobtype-是什么概念)
     - [Insert 的两个触发点](#insert-的两个触发点)
     - [blobType 是什么](#blobtype-是什么)
   - [问题 4：对外暴露的 API 中，batch_alloc 和 batch_put_from 有什么区别？](#问题-4对外暴露的-api-中batch_alloc-和-batch_put_from-有什么区别)
   - [问题 5：Get 路径中对 blob 的读取是不是在 NPU 进程上触发的？同步执行 RewarmBlob 会不会很花时间、影响推理性能？](#问题-5get-路径中对-blob-的读取是不是在-npu-进程上触发的同步执行-rewarmblob-会不会很花时间影响推理性能)
     - [读取在哪里触发](#读取在哪里触发)
     - [会不会很慢、影响推理？](#会不会很慢影响推理)
   - [问题 6：Get 路径中的"高层"是什么含义？一定是 HBM 吗？"低层伴生（写入进行中）"是什么、如何判断？](#问题-6get-路径中的高层是什么含义一定是-hbm-吗低层伴生写入进行中是什么如何判断)
     - ["高层"≠ 特指 HBM](#高层-特指-hbm)
     - ["低层伴生"是什么、怎么判断](#低层伴生是什么怎么判断)
   - [问题 7：MmcMemObjMeta 对象的结构是什么样的，功能是什么？](#问题-7mmcmemobjmeta-对象的结构是什么样的功能是什么)
   - [问题 8："pendingBlob 有 lowerBlob 伴生 → 回温中可等；没有伴生 → 写入中对读不可见"如何理解？](#问题-8pendingblob-有-lowerblob-伴生--回温中可等没有伴生--写入中对读不可见如何理解)
     - [场景 A：pendingBlob **有** lowerBlob 伴生 → 回温进行中，值得等](#场景-apendingblob-有-lowerblob-伴生--回温进行中值得等)
     - [场景 B：pendingBlob **没有** lowerBlob 伴生 → 首次写入进行中，不能等](#场景-bpendingblob-没有-lowerblob-伴生--首次写入进行中不能等)
     - [一句话对比](#一句话对比)
9. [Q&A：MemcacheBackend 各方法在请求处理中的调用时序](#qamemcachebackend-各方法在请求处理中的调用时序)
   - [一、总体时序图](#一总体时序图)
   - [二、必要说明](#二必要说明)
     - [1. 调用次数统计（一个"命中且需要回存"的请求，非 layerwise）](#1-调用次数统计一个命中且需要回存的请求非-layerwise)
     - [2. 三条路径对应的角色](#2-三条路径对应的角色)
     - [3. lazy_init（DSV4 压缩模型）对时序的改变](#3-lazy_initdsv4-压缩模型对时序的改变)
     - [4. 线程模型](#4-线程模型)
     - [5. 容错语义](#5-容错语义)
10. [Q&A：基础概念澄清（num_kv_head / TP rank / 三种情形 / 注意力结构 / update_block_hashes / 哈希函数 H）](#qa基础概念澄清num_kv_head--tp-rank--三种情形--注意力结构--update_block_hashes--哈希函数-h)
    - [1. `num_kv_head` 是什么意思？](#1-num_kv_head-是什么意思)
    - [2. TP rank 是什么概念？为什么每个 TP rank 的 NPU 上有自己的一份 KV cache 显存？](#2-tp-rank-是什么概念为什么每个-tp-rank-的-npu-上有自己的一份-kv-cache-显存)
    - [3. 三个情形分别在什么使用场景出现？](#3-三个情形分别在什么使用场景出现)
    - [4. MHA / GQA / MLA 分别是什么注意力结构？](#4-mha--gqa--mla-分别是什么注意力结构)
    - [5. `update_block_hashes()` 详解 + 它在请求链中的位置](#5-update_block_hashes-详解--它在请求链中的位置)
      - [代码本体（`vllm/v1/request.py`）](#代码本体vllmv1requestpy)
      - [为什么说是"增量补齐"](#为什么说是增量补齐)
      - [在请求处理链中的位置](#在请求处理链中的位置)
    - [6. `H` 是 hash 的意思吗？](#6-h-是-hash-的意思吗)
11. [Q&A：Scheduler.schedule() 与 KVCacheManager 的协同工作](#qaschedulerschedule-与-kvcachemanager-的协同工作)
    - [一、角色分工总览](#一角色分工总览)
    - [二、`schedule()` 主流程与 KVCacheManager 的触点](#二schedule-主流程与-kvcachemanager-的触点)
    - [三、`allocate_slots` 内部：管家的一次完整分配](#三allocate_slots-内部管家的一次完整分配)
    - [四、schedule() 之外：一轮完整生命周期中的配合](#四schedule-之外一轮完整生命周期中的配合)
    - [五、和 memcache / AscendStore 如何衔接](#五和-memcache--ascendstore-如何衔接)
    - [六、设计要点提炼](#六设计要点提炼)
12. [put_from 完整流程 与 KV cache 的优先落点](#put_from-完整流程-与-kv-cache-的优先落点)
    - [问题：调用 memcache 的 put_from 接口的完整流程是怎样的？会优先把 KV cache 张量存在哪里？](#问题调用-memcache-的-put_from-接口的完整流程是怎样的会优先把-kv-cache-张量存在哪里)
      - [一、put_from 完整流程（跨 3 个进程/模块）](#一put_from-完整流程跨-3-个进程模块)
      - [二、KV cache 会优先存在哪里？](#二kv-cache-会优先存在哪里)
13. [Q&A：kv_connector_model_runner_mixin 中的 forward 含义与流程](#qakv_connector_model_runner_mixin-中的-forward-含义与流程)
    - ["forward" 的含义](#forward-的含义)
    - [forward 前后都做什么（`gpu_model_runner.py:4416-4440`）](#forward-前后都做什么gpu_model_runnerpy4416-4440)
14. [Q&A：ModelRunner 连接器钩子（Mixin）如何生效、用了什么 Python 语法](#qamodelrunner-连接器钩子mixin如何生效用了什么-python-语法)
    - [1. 生效方式：多重继承（Mixin 模式）](#1-生效方式多重继承mixin-模式)
    - [2. 语法点一：`@staticmethod`——无状态的工具方法](#2-语法点一staticmethod无状态的工具方法)
    - [3. 语法点二：`@contextmanager`——生成器变成上下文管理器](#3-语法点二contextmanager生成器变成上下文管理器)
    - [4. 一个配套技巧：`nullcontext` 兜底](#4-一个配套技巧nullcontext-兜底)
    - [总结](#总结)
15. [Q&A：_try_schedule_encoder_inputs 方法的请求与返回值含义详解](#qa_try_schedule_encoder_inputs-方法的请求与返回值含义详解)
    - [一、方法定位](#一方法定位)
    - [二、输入参数（5 个）](#二输入参数5-个)
    - [三、返回值（4 元组）](#三返回值4-元组)
    - [四、内部决策逻辑（一张图的"命运"如何决定）](#四内部决策逻辑一张图的命运如何决定)
    - [五、实例演示](#五实例演示)
    - [六、一句话总结](#六一句话总结)
16. [Q&A：已命中的 encoder 缓存在 GPUModelRunner 中如何拉取](#qa已命中的-encoder-缓存在-gpumodelrunner-中如何拉取)
    - [一、整体图景：每个 worker 一本"实物账"](#一整体图景每个-worker-一本实物账)
    - [二、逐步拆解：命中缓存的"拉取"过程](#二逐步拆解命中缓存的拉取过程)
    - [三、完整时序图](#三完整时序图)
    - [四、要点提炼](#四要点提炼)
